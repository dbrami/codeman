---
description: Install destrier's critical-path pre-commit guard into the current git repo.
---
...
mkdir -p .destrier
if [ ! -f .destrier/critical-paths.txt ]; then
  cat > .destrier/critical-paths.txt <<'EOF'
...
echo "Installed. Edit .destrier/critical-paths.txt to define your critical paths."
```

Set `DESTRIER_PRECOMMIT_BLOCK=1` in the environment to make the guard block
